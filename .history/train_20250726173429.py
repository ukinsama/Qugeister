...
class Env_Geister:
    ...
    def start_training(self, episodes, visualize_interval=0, train_agents=True, model_save_interval=100, model_dir_prefix="./models_geister",model_type="CQCNN"):
        ...
        for i in range(episodes):
            ...
            if train_agents and model_save_interval > 0 and (i + 1) % model_save_interval == 0:
                save_agent(self.agent1, f"{model_dir_prefix}_agentA/eps{i+1}", model_type=model_type)
                if self.agent2 != self.agent1:
                    save_agent(self.agent2, f"{model_dir_prefix}_agentB/eps{i+1}", model_type=model_type)
...
def run_geister_cnn_training(episodes=100):
    ...
    env.start_training(
        ...
        train_agents=True,
        model_save_interval=episodes // 5,
        model_type="CNN"
    )
...
def run_geister_cqcnn_training(episodes=100, n_qbits=4, cnn_out_feat=4, game_instance=None):
    ...
    input_channels = 6  # 状態表現のチャネル数 (GeisterGame.get_state の出力に合わせる)
    # 例: 自良(1),自悪(1),敵駒(1),自駒位置(1),自脱出口(1),敵脱出口(1) -> 6チャネル
    ...
    log = env_q.start_training(episodes, visualize_interval=0, train_agents=True, model_save_interval=20,model_type="CQCNN")
