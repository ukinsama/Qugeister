...
class CNNAgent_Geister(Agent):
    def __init__(self, player_id:str, game:GeisterGame, network_name="CCNN2_Geister", 
                 board_size=BOARD_SIZE, input_channels=6, elo=1500, epsilon=0.8, lr=0.001): # input_channelsを6に変更
        super().__init__(player_id, game)
        self.discount = 0.99
...
def play_one_game_with_log(self, visualize=False, train_agents=True):
    ...
    for turn_count in range(self.max_turns_per_game):
        if visualize:
            self.game.display_board(reveal_opponent_pieces=True)
        ...
        if not possible_moves:
            ...
            # 勝者側のupdate
            if train_agents:
                if opponent_agent == self.agent1 and last_info_agent1["state"] is not None:
                    loss = self.agent1.update(
                        last_info_agent1["state"], last_info_agent1["action"], 1.0, current_state_tensor, True)
                    if loss is not None:
                        self.loss_history_agent1.append(loss)
                elif opponent_agent == self.agent2 and last_info_agent2["state"] is not None:
                    loss = self.agent2.update(
                        last_info_agent2["state"], last_info_agent2["action"], 1.0, current_state_tensor, True)
                    if loss is not None:
        ...
        # opponent update (pending update)
        if train_agents:
            info_to_update = last_info_agent1 if active_agent == self.agent2 else last_info_agent2
            if info_to_update["state"] is not None:
                reward_for_opponent = 0.0 if not done_active else -1.0
                loss = (self.agent1.update if active_agent == self.agent2 else self.agent2.update)(
                    info_to_update["state"], info_to_update["action"], reward_for_opponent, current_state_tensor, done_active)
                if loss is not None:
                    (self.loss_history_agent1 if active_agent == self.agent2 else self.loss_history_agent2).append(loss)

        if train_agents:
            loss = active_agent.update(
                last_info_agent1["state"] if active_agent == self.agent1 else last_info_agent2["state"],
                last_info_agent1["action"] if active_agent == self.agent1 else last_info_agent2["action"],
                reward_active, next_state_tensor_for_active, done_active
            )
            if loss is not None:
                (self.loss_history_agent1 if active_agent == self.agent1 else self.loss_history_agent2).append(loss)
