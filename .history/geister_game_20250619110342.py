...
class GeisterGame:
    def __init__(self, board_size=6, num_ghosts_per_player=8, good_ghost="G", bad_ghost="B", empty_symbol=".", player_a_id="A", player_b_id="B", exits_a=None, exits_b=None, setup_strategy_a=None, setup_strategy_b=None):
        ...
        self.GOOD_GHOST = good_ghost
        self.BAD_GHOST = bad_ghost
        self.EMPTY = empty_symbol
        self.PLAYER_A_ID = player_a_id
        self.PLAYER_B_ID = player_b_id
        self.PLAYER_A_GOOD = self.get_piece_str(self.PLAYER_A_ID, self.GOOD_GHOST)
        self.PLAYER_A_BAD = self.get_piece_str(self.PLAYER_A_ID, self.BAD_GHOST)
        self.PLAYER_B_GOOD = self.get_piece_str(self.PLAYER_B_ID, self.GOOD_GHOST)
        self.PLAYER_B_BAD = self.get_piece_str(self.PLAYER_B_ID, self.BAD_GHOST)
    ...
    def get_player_of_piece(self, piece_str):
        if piece_str != self.EMPTY and len(piece_str) > 0: return piece_str[0]
        return None

    def get_kind_of_piece(self, piece_str):
        if piece_str != self.EMPTY and len(piece_str) > 1: return piece_str[1]
        return None
    ...
    def make_move(self, from_pos, to_pos):
        ...
        if self.get_kind_of_piece(moving_piece_str) == GOOD_GHOST and (to_r, to_c) in (self.player_a_exits if self.current_player == self.PLAYER_A_ID else self.player_b_exits):
    ...
    def check_win_condition(self):
        ...
        if stats_a["captured_good"] >= win_threshold:
            self.game_over = True
            self.winner = PLAYER_A_ID
        elif stats_b["captured_good"] >= win_threshold:
            self.game_over = True
            self.winner = PLAYER_B_ID
    ...
    def checkwinner_for_reward(self, player_id):
        ...
        if good_positions:
            my_exits = self.player_a_exits if player_id == PLAYER_A_ID else self.player_b_exits
            min_distance = min(
