...
class GeisterGame:
    ...
    def make_move(self, from_pos, to_pos):
        ...
        # --- 修正部分 ---
        moving_kind = self.get_kind_of_piece(moving_piece_str)
        if self.current_player == self.PLAYER_A_ID:
            if moving_kind == self.GOOD_GHOST_A and (to_r, to_c) in self.player_a_exits:
            ...
            if moving_kind == self.GOOD_GHOST_B and (to_r, to_c) in self.player_b_exits:
                self.player_stats[self.current_player]["escaped_good"] += 1
                self.board.remove_piece(to_r, to_c)
                self.player_stats[self.current_player]["pieces_left"] -= 1
        # ------------------
