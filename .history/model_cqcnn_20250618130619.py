...
from geister_game import BOARD_SIZE  # BOARD_SIZEはgeister_game.pyからインポート
...
class CNN_QNN_CNN_Geister(nn.Module):
    ...
    def visualize_internal_flow(self, input_state_tensor: torch.Tensor):
        """
        モデルの内部の流れ（CNN出力、CNN->QNN入力、QNN回路、Q値マップ）を可視化。
        input_state_tensor: torch.Tensor (channels, H, W)
        """
        self.HNN.eval()
        with torch.no_grad():
            # CNN 特徴抽出部の出力
            cnn_features = self.HNN.cnn_feature_extractor(input_state_tensor.unsqueeze(0)).squeeze(0)
            cnn_features_np = cnn_features.cpu().numpy()

        # CNN の特徴マップ表示
        num_features = cnn_features_np.shape[0]
        fig, axes = plt.subplots(1, num_features, figsize=(3*num_features, 3))
        if num_features == 1:
            axes = [axes]
