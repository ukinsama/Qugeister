...
def save_cnn_agent(agent, dir_path: str, name: str = "agent"):
    """
    CNNモデルの重みとコンフィグを保存する関数。
    """
    os.makedirs(dir_path, exist_ok=True)
    model = agent.NN  # CNNAgent_Geister の NN を想定
    torch.save(model.state_dict(), os.path.join(dir_path, f"{name}_weights.pth"))
    print(f"💾 Saved weights: {name}_weights.pth")

    config = {
        "input_channels": getattr(model, "input_channels", None),
        "board_size": getattr(model, "board_size", None),
        "cnn_fc_out_features": getattr(model, "cnn_fc_out_features", None)
    }
    with open(os.path.join(dir_path, f"{name}_config.json"), "w") as f:
        json.dump(config, f, indent=2)
    print(f"📝 Saved config: {name}_config.json")

def load_cnn_agent(agent_class, dir_path: str, name: str = "agent"):
    """
    保存されたCNNモデルの重みとコンフィグを読み込む。
    """
    config_path = os.path.join(dir_path, f"{name}_config.json")
    with open(config_path, "r") as f:
        config = json.load(f)

    model = agent_class(config.get("input_channels"), config.get("board_size"))
    weight_path = os.path.join(dir_path, f"{name}_weights.pth")
    model.NN.load_state_dict(torch.load(weight_path))
    print(f"✅ Loaded CNN model from {weight_path}")
    return model