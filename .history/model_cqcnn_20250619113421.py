...
class CCNN2_Geister(nn.Module):
    def __init__(self, input_channels=6, board_size=None, num_outputs=None):
        if board_size is None:
            raise ValueError("board_size must be provided explicitly")
        if num_outputs is None:
            num_outputs = board_size * board_size
        super().__init__()
        self.board_size = board_size
        self.conv = nn.Conv2d(input_channels, 32, kernel_size=3, padding=1)
        self.fc = nn.Linear(32 * board_size * board_size, num_outputs)
